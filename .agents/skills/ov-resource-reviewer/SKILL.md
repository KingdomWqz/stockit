...
本 Skill 是 OpenViking **Resource 的 reviewer**：对 `viking://resources/...` 下的目录结构、命名、重复、L0/L1 一致性、索引健康做**只读审查**，输出一份**审查报告**。
...
- **按需加载 L0/L1/L2**：
  ...
  - L2 原始文件与子目录：完整详情。
  - 这些特殊文件由系统在「新增数据」或「session 归档」时**自底向上聚合**自动生成（叶子节点先有 L0/L1，再聚合到父级）；多模态内容（图片/视频）也会生成文本描述用于统一检索。
...
| 维度 | 检查点 |
|------|--------|
| **目录语义规范性** | 目录名是否承载明确业务语义？是否存在中文长后缀、临时名、`(1)`/`_copy`/时间戳等自动生成痕迹？ |
| **重复目录** | 是否出现同名带后缀的重复目录？官方现象：全量导入时同名目录已存在，系统自动建带后缀新目录，旧目录内容不完整。 |
| **目录层级与归属** | 是否挂在正确的 scope / 父目录下？一个 Resource 目录是否对应一个项目/一套文档/一个主题？ |
| **L0/L1 完整性与一致性** | `.abstract.md` / `.overview.md` 是否已生成？内容是否与目录当前内容一致（删改后未刷新会漂移）？ |
| **索引健康** | 语义/向量是否可被检索（`find` 能否命中预期关键词）？ |
...
```bash
# 1. 侦察：看目录树（含隐藏的 L0/L1 特殊文件）
ov tree viking://resources/<root> -L 4 -a
...
> **读取预算**：遵循 L0 → L1 → L2 的渐进加载。先扫多个 L0（每个 ~100 token）过滤，再读最相关的 L1，最后才按需 read L2，避免直接全量读 L2 浪费 token。

> **异步状态提示**：若 `ov task list` 显示有未完成的异步处理，索引与 L0/L1 可能尚未刷新，此时审查结论必须标注「索引可能未刷新，建议 `ov wait` 后再审」。
...
## 问题清单
每条：
- 维度：<目录语义/重复目录/层级归属/L0L1一致性/索引健康/关系>
- URI：viking://...
- 现状：...
- 风险：...
...
|---|---|
| `ov tree <uri> -L <n> -a` | `ov mkdir <uri> --description "<desc>"` |
| `ov ls <uri>` | `ov mv <from-uri> <to-uri>` |
| `ov stat <uri>` | `ov rm <uri> -r --recursive --wait` |
| `ov abstract <uri>`（L0） | `ov reindex <uri> --mode semantic_and_vectors --wait true` |
| `ov overview <uri>`（L1） | `ov wait --timeout <s>` |
...
1. `ov tree viking://resources/stockit/docs -L 4 -a` 侦察。
2. `ov abstract viking://resources/stockit/docs/database/股票指标...个人本地版` 读 L0，判断语义与是否为重复目录。
