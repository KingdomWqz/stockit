...
def _seed_active_stocks(con: sqlite3.Connection, codes):
    con.executemany(
        "INSERT OR IGNORE INTO stocks (code, name, is_active) VALUES (?, ?, 1)",
        [(c, f"股票{c}") for c in codes],
    )
    con.commit()
...
def _quotes_rows(con: sqlite3.Connection) -> list[dict]:
    return [
        dict(r) for r in con.execute(
            "SELECT code, trade_date, adjust FROM stock_daily_quotes"
        ).fetchall()
    ]


# --------------------------------------------------------------------------- #
def test_bulk_sync_rejects_inverted_dates(client):
    resp = client.post(
...
def test_bulk_sync_rejects_range_over_5_days(client):
    # 6-day span -> 422
    resp = client.post(
        "/svc/api/stocks/daily-quotes/sync",
...
def test_bulk_sync_accepts_5_day_range(client, monkeypatch):
...
def test_bulk_sync_returns_202_and_pending_status(client, monkeypatch):
    monkeypatch.setattr(stocks, "_run_bulk_sync", lambda *a, **kw: None)
    resp = client.post(
        "/svc/api/stocks/daily-quotes/sync",
    ...
    assert client.get(
        "/svc/api/stocks/daily-quotes/sync/nope"
    ).status_code == 404
...
def test_bulk_sync_happy_path_completes(client, fake_db, monkeypatch):
    ...
    status = client.get(
        f"/svc/api/stocks/daily-quotes/sync/{job_id}"
    ).json()
